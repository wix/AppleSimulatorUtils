...
@implementation SetCriticalAlertsPermission

+ (BOOL)setCriticalAlertsStatus:(NSString*)status forBundleIdentifier:(NSString*)bundleIdentifier displayName:(NSString*)displayName simulatorIdentifier:(NSString*)simulatorId error:(NSError**)error
{
	LNLog(LNLogLevelDebug, @"Setting critical alerts permission");
...
+ (BOOL)_setSectionInfoData:(id)sectionInfoData forBundleIdentifier:(NSString*)bundleIdentifier displayName:(NSString*)displayName simulatorIdentifier:(NSString*)simulatorId error:(NSError**)error
{
	...
	if(error)
	{
		*error = [NSError errorWithDomain:@"SetCriticalAlertsPermissionError" code:0 userInfo:@{NSLocalizedDescriptionKey: @"BulletinBoard property list not found."}];
	}
